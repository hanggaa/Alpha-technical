...
function openZoom(container) {
  ...
  if (!img) return; // Prevent zooming if no image exists yet
  
  const overlay = document.getElementById('zoomOverlay');
  const content = document.getElementById('zoomContent');
  
  content.innerHTML = `<img id="zoomedImg" src="${img.src}" alt="Zoomed Diagram">`;
...
document.getElementById('zoomOverlay').addEventListener('click', (e) => {
  if (e.target === document.getElementById('zoomOverlay') || e.target.className === 'zoom-content') {
    closeZoom();
  }
});
